...
def run_feature_extraction(config, overwrite=False):
    ...
    if os.path.exists(config.feature_dir) and not overwrite:
        feature_files = len([f for f in os.listdir(config.feature_dir) if f.endswith('.npy')])
        if feature_files > 0:
            print(f"Found {feature_files} existing feature files in {config.feature_dir}")
...
def main():
    ...
    if not args.skip_stage1:
        stage1_checkpoint_path = run_stage1_training(config, force_retrain=args.force_retrain)
        ...
        stage1_best_path = os.path.join(config.checkpoint_dir, 'stage1', 'stage1_best.pth')
        if os.path.exists(stage1_best_path):
