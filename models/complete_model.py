...
from mtn_anomaly import MultipleTemporalNetwork, AnomalyScoreGenerator, AnomalyDetectionLoss
...
class R3D_MTN_AnomalyDetector(nn.Module):
    ...
    def __init__(self, 
        ...
        self.mtn = MultipleTemporalNetwork(
            feature_channels_dict=self.feature_channels,
            hidden_dim=mtn_hidden_dim
        )
    ...
    def predict_anomaly(self, x, threshold=0.5): # ngưỡng để phân loại bất thường 
        ...
        with torch.no_grad():
            ...
            return {
                ...
                'classification_probs': cls_probs.cpu().numpy() if cls_probs is not None else None,
                'classification_prediction': cls_predictions.cpu().numpy() if cls_predictions is not None else None,
                'attention_weights': results['attention_weights'].cpu().numpy() if results['attention_weights'] is not None else None
            }
...
if __name__ == "__main__":
    ...
    # Test input
    x = torch.randn(2, 3, 32, 224, 224).to(device)  # (batch, channels, time, height, width)
...
class CompleteModel(nn.Module):
    ...
    def __init__(self, backbone_type="r2plus1d_18", num_classes=2, pretrained=True, feature_extract_mode=True):
