    ...
    def __init__(self, in_channels, out_channels, temporal_scales=[1, 2, 4, 8]):
        ...
        for scale in temporal_scales:
            branch = nn.Sequential(
                nn.Conv3d(
                    in_channels,
                    out_channels // len(temporal_scales),
...
class MultipleTemporalNetwork(nn.Module):
    ...
    def __init__(self, feature_channels_dict, hidden_dim=512):
        ...
        for level, channels in feature_channels_dict.items():
            self.temporal_pyramids[level] = TemporalPyramidModule(
                in_channels=channels, out_channels=hidden_dim, temporal_scales=[1, 2]
            )
    ...
    def forward(self, features_dict):
        ...
        fused_features = self.level_fusion(
            combined_features
        )  # (B, hidden_dim, T, 1, 1)
...
class AnomalyScoreGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.feature_transform = nn.Sequential(
            *layers
        )  # (B*T, hidden_dim) B batch size, T time steps
        ...
        self.classifier = nn.Sequential(nn.Linear(prev_dim, num_classes))

        if use_mil:
            # Multiple Instance Learning aggregation
            self.mil_attention = nn.Sequential(
                nn.Linear(prev_dim, 128), nn.Tanh(), nn.Linear(128, 1)
            )

    def forward(
        self, temporal_features, return_scores=True, return_classification=True
    ):
        ...
        if return_scores:
            ...
            if self.use_mil:
                ...
                video_features = torch.sum(
                    mil_features * attention_weights, dim=1
                )  # (B, hidden_dim)
            ...
            results["attention_weights"] = (
                attention_weights.squeeze(-1) if self.use_mil else None
            )
...
class AnomalyDetectionLoss(nn.Module):
    ...
    def __init__(
        self, anomaly_weight=1.0, classification_weight=1.0, smoothness_weight=0.1
    ):
    ...
    def forward(self, predictions, targets):
        ...
        if "classification_logits" in predictions and "labels" in targets:
            cls_loss = self.ce_loss(
                predictions["classification_logits"], targets["labels"]
            )
        ...
        if "anomaly_scores" in predictions and self.smoothness_weight > 0:
            ...
            smoothness_loss = torch.mean(
                torch.abs(frame_scores[:, 1:] - frame_scores[:, :-1])
            )
