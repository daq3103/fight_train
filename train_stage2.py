...
class Stage2Trainer(R3D_MTN_Trainer):
    ...
    def _generate_clip_pseudo_labels(self, features_batch, video_labels_batch):
        ...
        for features, video_label in zip(features_batch, video_labels_batch):
            ...
            pseudo_labels_tensor = torch.from_numpy(pseudo_labels).long().to(self.device)
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            features_list = batch_data['features_list']  # List of variable length tensors
            video_labels = batch_data['labels']          # (B,) video labels
            
            batch_size = len(features_list)
            
            # Generate pseudo labels cho clips
            pseudo_labels_batch = self._generate_clip_pseudo_labels(features_list, video_labels)
            ...
            for features, pseudo_labels in zip(features_list, pseudo_labels_batch):
                ...
                if clip_logits is None:
                    ...
                    clip_logits = torch.randn(1, features.size(0), 2, device=self.device)
    ...
    def evaluate(self, test_loader):
        ...
        with torch.no_grad():
            for batch_data in test_loader:
                ...
                for i, (features, pseudo_labels) in enumerate(zip(features_list, pseudo_labels_batch)):
        ...
        predictions = {'video_score': torch.tensor(all_video_scores, device=self.device)}
...
def main():
    ...
    parser.add_argument('--max_clips', type=int, default=128,
                       help='Maximum clips per video')
