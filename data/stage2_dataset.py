...
class Stage2Dataset(Dataset):
    ...
    def __init__(self, feature_dir: str, data_root: str, mode: str = 'train',
                 feature_dim: int = 2048, min_clips: int = 8, max_clips: int = 256):
        """
        Args:
        ...
        print(f"  Clips per video: min={min(clip_counts)}, max={max(clip_counts)}, mean={np.mean(clip_counts):.1f}")
    ...
    def _load_video_data(self) -> List[dict]:
        ...
        for video_path in video_files:
            ...
            feature_file = os.path.join(self.feature_dir, video_name.replace('.mp4', '.npy'))
            ...
            try:
                ...
                if num_clips > self.max_clips:
                    print(f"Truncating {video_name}: {num_clips} -> {self.max_clips} clips")
...
def create_stage2_dataloaders(feature_dir: str, data_root: str, batch_size: int = 4,
                             feature_dim: int = 2048, min_clips: int = 8,
                             max_clips: int = 256, num_workers: int = 2) -> Tuple[DataLoader, DataLoader]:
    """
    ...
    def generate_pseudo_labels(self, features, video_label, threshold=0.5, top_k=None):
        ...
        with torch.no_grad():
            ...
            if isinstance(model_outputs, dict):
                clip_scores = model_outputs.get('clip_scores', model_outputs.get('anomaly_scores'))
            else:
            ...
            if self.strategy == 'threshold':
                ...
                pseudo_labels = self._uniform_pseudo_labels(len(clip_scores), video_label)
    ...
    if os.path.exists(feature_dir):
        ...
        if len(dataset) > 0:
            ...
            for batch in train_loader:
                print(f"Batch features: {batch['features'].shape}")      # (B, max_clips, feature_dim)
                print(f"Batch attention_mask: {batch['attention_mask'].shape}")  # (B, max_clips)
