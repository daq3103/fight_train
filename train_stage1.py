...
class Stage1Trainer(R3D_MTN_Trainer):
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            # Get data
            normal_features = batch_data['normal_features'].to(self.device)    # (B, N, feature_dim)
            abnormal_features = batch_data['abnormal_features'].to(self.device) # (B, N, feature_dim)
            ...
            sparsity_loss = self._sparsity_loss(torch.cat([normal_scores, abnormal_scores], dim=0))
            smoothness_loss = self._smoothness_loss(torch.cat([normal_scores, abnormal_scores], dim=0))
        ...
        with torch.no_grad():
            for batch_data in test_loader:
                ...
                if isinstance(normal_outputs, dict):
                    normal_scores = normal_outputs.get('video_score', torch.max(normal_outputs.get('anomaly_scores', normal_outputs), dim=-1)[0])
                    abnormal_scores = abnormal_outputs.get('video_score', torch.max(abnormal_outputs.get('anomaly_scores', abnormal_outputs), dim=-1)[0])
                else:
                ...
                ranking_loss = self._ranking_loss(normal_scores.unsqueeze(1), abnormal_scores.unsqueeze(1))
                ...
                targets = {
                    'labels': torch.cat([
                        torch.zeros(batch_size, dtype=torch.long, device=self.device),  # Normal = 0
                        torch.ones(batch_size, dtype=torch.long, device=self.device)    # Abnormal = 1
                    ], dim=0)
                }
    ...
    def save_checkpoint(self, epoch, metrics, checkpoint_dir):
        ...
        super().save_checkpoint(filepath, epoch, best_loss=metrics.get('loss', float('inf')))
    ...
    def train(self, train_loader, test_loader, num_epochs, checkpoint_dir='./checkpoints'):
        """Main training loop"""
...
def main():
    ...
    parser.add_argument('--feature_dir', type=str, default='./features_i3d',
                       help='Directory containing I3D features')
    parser.add_argument('--data_root', type=str, default='./new_youtube',
