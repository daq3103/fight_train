...
def visualize_predictions(predictions, targets, save_dir=None):
    ...
    for i, (v_score, f_scores) in enumerate(zip(video_scores, frame_scores)):
        ...
        plt.subplot(3, 1, 1)
        plt.bar(
            ["Video"], [v_score], color="red" if v_score > 0.5 else "green", alpha=0.7
        )
        ...
        plt.fill_between(
            ...
            0,
            1,
            where=np.array(f_scores) > 0.5,
            alpha=0.3,
            color="red",
            label="Anomalous Frames",
        )
...
def create_video_with_annotations(
    video_path, frame_scores, output_path, threshold=0.5, fps=None
):
...
if __name__ == "__main__":
    ...
    train_losses = [
        1.0 - 0.8 * np.exp(-i / 10) + 0.1 * np.random.random() for i in range(50)
    ]
    val_losses = [
        1.0 - 0.7 * np.exp(-i / 12) + 0.15 * np.random.random() for i in range(50)
    ]
